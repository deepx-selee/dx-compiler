# DEEPX DX-COM 컴파일러 — AI 코딩 에이전트 진입점

> AI 코딩 에이전트를 위한 자체 완결형 진입점. 이 파일을 먼저 읽으세요.
...
| 명령 | 설명 |
|---------|-------------|
| `/dx-agent-compiler-convert` | PyTorch 모델을 ONNX로 변환 |
| `/dx-agent-compiler-compile` | ONNX 모델을 DXNN으로 컴파일 |
| `/dx-agent-compiler-validate` | 컴파일 출력 검증 |
| `/dx-swe-brainstorm` | 브레인스토밍, 2-3가지 접근법 제안, 스펙 자체 검토 후 계획 |
| `/dx-swe-tdd` | 검증 주도 개발, 선택적 Red-Green-Refactor 단위 테스트 |
| `/dx-swe-verify` | 완료 선언 전 검증 — 주장 전에 증거 |
...
13. **필수 브레인스토밍 질문**: 모든 컴파일 작업 전에 에이전트는 반드시 세 가지 필수 질문을 해야 함: (Q1) YOLO 버전 특성과 NMS-free 모델 감지, (Q2) 장단점과 ONNX simplification, (Q3) 설명과 PPU 컴파일 지원. 전체 세부사항은 `.deepx/agents/dx-compiler-builder.md` Step 2 참조.
14. **PPU 기본값은 OFF**: PPU 컴파일은 opt-in. 브레인스토밍 Q3에서 사용자가 명시적으로 확인한 경우에만 PPU config 추가. **YOLO26은 PPU를 지원하지 않음** — YOLO26의 경우 Q3 생략.
15. **모델 획득 — 지시가 아닌 다운로드 및 컴파일**: 사용자가 로컬 `.pt`/`.pth`/`.onnx` 파일을 제공하지 않은 경우, 에이전트는 공식 다운로드 소스 (Ultralytics releases, torchvision, timm, ONNX Model Zoo, Hugging Face)를 찾아 실제로 모델을 다운로드하고, 전체 파이프라인을 통해 `.dxnn` 파일을 생성하도록 컴파일해야 합니다. config.json만 생성하거나 컴파일 지시사항을 제공하고 멈추지 절대 마세요.
16. **컴파일 후 검증은 필수 — 검증 없이는 컴파일이 완료되지 않음**: 모든 성공적인 `dxcom` 컴파일 후, 결과를 제시하기 전에 에이전트는 반드시 다음을 모두 완료해야 합니다. 이것 없이 "컴파일 성공" 요약을 절대 제시하지 마세요:
    - **(a)** 출력 디렉토리 (세션 디렉토리 또는 사용자 지정 디렉토리)에 `setup.sh`, `run.sh`, `README.md` 생성
    - **(b)** `verify.py` 생성 — ONNX vs DXNN 추론 비교
    - **(c)** `verify.py` 실행 및 PASS 확인
    - **(d)** `${WORK_DIR}/session.log`에 세션 로그 저장 — 손으로 작성한 요약이 아닌 `tee`를 통해 캡처된 **실제 명령 실행 출력**이어야 함
    - **(e)** 최종 요약에 검증 결과 및 모든 결과물 경로 포함
    - **사용자가 커스텀 출력 디렉토리를 지정한 경우에도** (예: `dx-agent-dev/` 대신 소스 디렉토리), 이러한 결과물은 여전히 필수입니다. 사용자가 다른 출력 경로를 선택하는 것이 배포 스크립트 생성을 면제하지 않습니다.
17. **이전 세션 결과물 재사용 금지**: `dx-agent-dev/`의 이전 세션에서 결과물을 확인, 나열, 탐색, 또는 재사용하지 절대 마세요. 각 컴파일 실행은 새로운 타임스탬프로 새 세션 디렉토리를 생성해야 합니다. 이전 세션이 동일한 모델을 컴파일한 경우에도 항상 처음부터 다시 다운로드, 다시 export, 다시 컴파일하세요. `ls dx-agent-dev/`를 실행하거나 이전 실행의 기존 `.onnx`/`.dxnn` 파일을 확인하지 마세요.
18. **setup.sh에서 venv는 필수**: 생성된 `setup.sh`는 `pip install` 전에 가상 환경을 생성하고 활성화해야 합니다. Ubuntu 24.04+에서는 PEP 668이 시스템 전체 pip 설치를 차단합니다. `${VIRTUAL_ENV:-}` 확인과 함께 `python3 -m venv`를 사용하세요. 생성된 `run.sh`는 venv 활성화를 확인하고 누락 시 자동 활성화하거나 에러를 출력해야 합니다.
19. **사전 컴파일된 참조 모델과의 교차 검증**: 동일한 모델에 대해 사전 컴파일된 DXNN이 `dx-runtime/dx_app/assets/models/`에 존재하는 경우, 사전 컴파일된 모델과 생성된 모델 모두에 대해 verify.py를 실행하세요 (Phase 5.7). 둘 다 실패 → verify.py 버그. 사전 컴파일된 것은 통과하고 생성된 것은 실패 → 컴파일 문제. `.deepx/agents/dx-dxnn-compiler.md` Phase 5.7 참조.
20. **NHWC/NCHW DataLoader 불일치**: dxcom CLI의 기본 dataloader는 NHWC `[1,H,W,C]`로 이미지를 로드합니다. ONNX 모델이 NCHW `[1,C,H,W]`를 기대하는 경우 (대부분의 PyTorch export 모델), CLI 컴파일은 `DataLoaderError: Input shape mismatch`로 실패합니다. **수정**: NCHW 텐서를 생성하는 커스텀 torch DataLoader와 함께 Python API (`dx_com.compile()`)를 사용하세요. `.deepx/memory/common_pitfalls.md` pitfall #18 참조.
...
샘플 JSON config는 config.json 생성 시 표준 참조로 사용됩니다.

## 출력 격리

모든 컴파일 결과물은 기본적으로 `dx-agent-dev/<session_id>/`에 저장됩니다. 각
