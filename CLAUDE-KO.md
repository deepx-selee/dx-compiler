...
| `/dx-swe-verify` | 프로세스: 완료 선언 전 검증 — 주장 전에 증거 |
...
14. **컴파일 후 검증은 필수 — 검증 없이는 컴파일이 완료되지 않음**: 모든 성공적인 `dxcom` 컴파일 후, 결과를 사용자에게 제시하기 전에 에이전트는 반드시 다음을 모두 완료해야 합니다. 이것 없이 "컴파일 성공" 요약을 절대 제시하지 마세요:
    ...
    - **사용자가 커스텀 출력 디렉토리를 지정한 경우에도** (예: `dx-agent-dev/` 대신 소스 디렉토리), 이러한 결과물은 여전히 필수입니다.
15. **이전 세션 결과물 재사용 금지**: `dx-agent-dev/`의 이전 세션에서 결과물을 확인, 나열, 탐색, 또는 재사용하지 절대 마세요. 각 컴파일 실행은 새로운 타임스탬프로 새 세션 디렉토리를 생성해야 합니다. 이전 세션이 동일한 모델을 컴파일한 경우에도 항상 처음부터 다시 다운로드, 다시 export, 다시 컴파일하세요. `ls dx-agent-dev/`를 실행하거나 이전 실행의 기존 `.onnx`/`.dxnn` 파일을 확인하지 마세요.
